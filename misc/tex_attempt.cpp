...
const int RUBER = 0, UNUM = 1, DUO = 2, PRIMUS = 3, SECUNDUS = 4,
	WARBIRD = 5, UNUM_SITE = 6, SECUNDUS_SITE = 7, PLAYER_MISSILE = 8,
	ENEMY_MISSILE = 9, BACKPLANE_01 = 10;
...
const GLuint nVerticesBackplane = 20;  // Backplane for star map.
...
// vectors for "Backplane"
glm::vec4 vertexBackplane[nVerticesBackplane];
glm::vec3 normalBackplane[nVerticesBackplane];
glm::vec4 diffuseColorMaterialBackplane[nVerticesBackplane];
...
// Create object for texture mapping.
static const GLfloat BACKPLANE_VERTICES_01[] = {
  -256.0f, -256.0f,  1.0f, 1.0f,  // 0 front left bottom 
   256.0f, -256.0f,  1.0f, 1.0f,  // 1 front right bottom
   256.0f, 256.0f, 1.0f, 1.0f,  // 2 back right bottom
  -256.0f, 256.0f, 1.0f, 1.0f,  // 3 back left bottom
   0.0f,  128.0f,  0.0f, 1.0f}; // 4 apex

// indexes to reuse vertex values
static const unsigned int indices[] = { // 6 faces, 3 vertices per face
  0, 1, 2, // Front Face 
  2, 3, 0}; // Back Bottom Face 
...
void init (void) {
	...
	for(int i = 0; i < nShapes; i++) {
		if (i < WARBIRD) {  // Initialize planetary bodies.
			...
			Model = glGetUniformLocation(shaderProgram, "ModelView");
			ViewProj = glGetUniformLocation(shaderProgram, "Projection");
		} else if (i == PLAYER_MISSILE || i == ENEMY_MISSILE) {  // Initialize missiles.
			...
			Model = glGetUniformLocation(shaderProgram, "ModelView");
			ViewProj = glGetUniformLocation(shaderProgram, "Projection");
		} else {  // Initialize backplanes for star maps.
			shaderProgram = loadShaders(vertexShaderFile, fragmentShaderFile);
			glUseProgram(shaderProgram);
			...
			glGenBuffers( 1, &(buffer[i]) );
			glBindBuffer( GL_ARRAY_BUFFER, buffer[i] );
			glBufferData( GL_ARRAY_BUFFER, sizeof(BACKPLANE_VERTICES_01) + sizeof(diffuseColorMaterialBackplane) + sizeof(normalBackplane), NULL, GL_STATIC_DRAW );
			glBufferSubData( GL_ARRAY_BUFFER, 0, sizeof(BACKPLANE_VERTICES_01), BACKPLANE_VERTICES_01 );
			glBufferSubData( GL_ARRAY_BUFFER, sizeof(BACKPLANE_VERTICES_01), sizeof(diffuseColorMaterialBackplane), diffuseColorMaterialBackplane );
			glBufferSubData( GL_ARRAY_BUFFER, sizeof(BACKPLANE_VERTICES_01) + sizeof(diffuseColorMaterialBackplane), sizeof(normalBackplane), normalBackplane );
			...
			vColor[i] = glGetAttribLocation( shaderProgram, "vColor" );
			glEnableVertexAttribArray( vColor[i] );
			glVertexAttribPointer( vColor[i], 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(vertexBackplane)) );

			vNormal[i] = glGetAttribLocation( shaderProgram, "vNormal" );
			glEnableVertexAttribArray( vNormal[i]);
			glVertexAttribPointer( vNormal[i], 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(vertexBackplane) + sizeof(diffuseColorMaterialBackplane)) );
...
void display(void) {
	...
	for (int i = 0; i < nShapes; i++) { 
		...
		else if (i == PLAYER_MISSILE || i == ENEMY_MISSILE)
			glDrawArrays(GL_TRIANGLES, 0, nVerticesMissile);  // Draw missiles.
		else
			glDrawArrays(GL_TRIANGLES, 0, nVerticesBackplane);  // Draw backplane for star map.
	}
